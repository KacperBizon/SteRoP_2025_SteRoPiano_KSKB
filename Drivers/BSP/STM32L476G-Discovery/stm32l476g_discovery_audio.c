...
// DODAJ TO: Dostęp do zmiennej z main.c
extern __IO int16_t UpdatePointer;
extern uint16_t PlayBuff[]; // Może być potrzebne, jeśli callbacki BSP używają bufora
#ifndef PLAY_BUFF_SIZE // Jeśli nie jest zdefiniowane w plikach BSP
#define PLAY_BUFF_SIZE 4096 // Upewnij się, że rozmiar jest taki sam jak w main.c
#endif
/**
  * @}
  */
...
{
  if(hsai->Instance == AUDIO_SAIx) // Sprawdź instancję
  {
    UpdatePointer = PLAY_BUFF_SIZE / 2; // Ustaw flagę dla main.c

    /* Oryginalny kod BSP - możesz go zostawić lub usunąć, jeśli nie używasz CbTransferComplete */
...
void HAL_SAI_TxHalfCpltCallback(SAI_HandleTypeDef *hsai)
{
  if(hsai->Instance == AUDIO_SAIx) // Sprawdź instancję
  {
     UpdatePointer = 0; // Ustaw flagę dla main.c

     /* Oryginalny kod BSP - możesz go zostawić lub usunąć, jeśli nie używasz CbHalfTransfer */
